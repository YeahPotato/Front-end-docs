...
##### [建议] `,` 和 `;` 前不允许有空格。如果不位于行尾，`,` 和 `;` 后必须跟一个空格。
...
##### [建议] 在 `if / else / for / do / while` 语句中，即使只有一行，也不得省略块 `{...}`。
...
##### [建议] `IIFE` 必须在函数表达式外添加 `(`，非 `IIFE` 不得在函数表达式外添加 `(`。
...
/*
* 求两个数字的和
* @param {Number} num1  第一个数字参数
* @param {Number} num2  第二个数字参数
